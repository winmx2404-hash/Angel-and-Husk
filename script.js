...
// SOLO UNA FUNCIÓN initJukebox (ELIMINADA LA DUPLICADA)
...
// ========== FONDOS CON VIDEOS POR CANCIÓN (VERSIÓN ÚNICA) ==========
...
function updateVinylCover(imageFile, songTitle) {
  ...
  const baseUrl = window.location.origin + window.location.pathname.replace(/\/[^/]*$/, '');
  const imageUrl = baseUrl + 'images/' + imageFile;
  
  console.log('🎨 Actualizando vinilo con:', imageUrl);
  
  // Crear imagen temporal para aplicar blur
  const img = new Image();
  img.crossOrigin = 'anonymous';
  img.src = imageUrl;
  
  img.onload = () => {
    // Crear canvas para aplicar blur
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    
    // Tamaño del canvas (igual que el vinilo)
    canvas.width = 200;
    canvas.height = 200;
    
    // Dibujar imagen redimensionada y con blur
    ctx.filter = 'blur(8px) brightness(0.8) contrast(1.2)';
    ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
    
    // Añadir un gradiente circular para efecto de vinilo
    ctx.filter = 'none';
    ...
    if (vinylLabel) {
      vinylLabel.textContent = songTitle.substring(0, 6).toUpperCase();
    }
  };
  ...
  img.onerror = () => {
    ...
    vinyl.style.background = ''; // Vuelve al gradiente original
